return {
    ...
    config = function()
        require("diffview").setup({
            use_icons = true,
            enhanced_diff_hl = false, -- 启用更精细的差异高亮
        ...
        vim.keymap.set("n", "<leader>gj", ":DiffviewFileHistory --follow %<CR>", { desc = "Open current file history" })
        vim.keymap.set(
            "n",
            "<leader>gk",
            ":DiffviewOpen HEAD %<CR>",
            { desc = "Check diff between HEAD and current buffer" }
        )
        vim.keymap.set("n", "<leader>g;", ":DiffviewClose<CR>", { desc = "Close diffview" })
